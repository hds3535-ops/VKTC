...
const VALID_TABS=["home","schedule","mypage","events","members","session","draw","ranking","accounting","profile","settings"];
const PUBLIC_NAV=[
  ["home","홈","⌂"],
  ["schedule","예정된 게임",""],
  ["mypage","MY PAGE",""],
...
function readStoredMemberIdentity(){
  try{
    const raw=sessionStorage.getItem("vktcMemberIdentityV2");
    if(!raw)return null;
...
const PUBLIC_CACHE_KEY="vktcPublicCacheV1";
const VKTC_CLUB_ID="33333333-3333-4333-8333-333333333333";
const CURRENT_CLUB_ID=VKTC_CLUB_ID;
...
function App(){
  ...
  const[memberPin,setMemberPin]=useState(()=>sessionStorage.getItem("vktcPersonalMemberPinV2")||"");
  ...
  const[clubPromo,setClubPromo]=useState(initialPublicCache?.clubPromo||{title:"VKTC",content:"Victoria Korean Tennis Club과 함께 즐겁고 매너 있는 테니스를 만들어갑니다."});
  ...
  function requireAdmin(){
    ...
    if(user&&adminAccessReady&&!isAdmin){
      alert("이 계정에는 VKTC 관리자 권한이 없습니다.");
      return false;
    }
    setLoginOpen(true);
    return false;
  ...
  async function loadClubPromo(){
    ...
    if(data){
      const next={title:data.title||"VKTC",content:data.content||""};
      setClubPromo(next);
      setPromoForm(next);
  ...
  async function authenticateMemberPin(){
    ...
    if(error||!data?.member_id){
      alert("VKTC 개인 PIN이 올바르지 않거나 아직 설정되지 않았습니다.");
      return null;
    }
    const identity={member_id:data.member_id,name:data.name};
    sessionStorage.setItem("vktcPersonalMemberPinV2",pin);
    sessionStorage.setItem("vktcMemberIdentityV2",JSON.stringify(identity));
    sessionStorage.removeItem("vktcMemberPin");
    sessionStorage.removeItem("vktcMemberUnlocked");
    setMemberPin(pin);
  ...
  function clearMemberAccess(){
    sessionStorage.removeItem("vktcPersonalMemberPinV2");
    sessionStorage.removeItem("vktcMemberIdentityV2");
    sessionStorage.removeItem("vktcMemberPin");
    sessionStorage.removeItem("vktcMemberUnlocked");
    setMemberPin("");
  ...
  async function saveMemberPersonalPin(e){
    ...
    flash(`${name} 회원의 VKTC 전용 PIN을 설정했습니다.`);
  }
    ...
    try{
      ...
      if(tab==="mypage"){
        ...
        if(msg.toLowerCase().includes("invalid personal pin")){
          alert("이 회원의 VKTC 개인 PIN 연결을 확인할 수 없습니다. 관리자에게 회원 목록에서 VKTC PIN을 다시 설정해달라고 요청해주세요.");
        }else{
  ...
  async function addMyRacket(e){
    ...
    if(error){
      ...
      if(msg.toLowerCase().includes("invalid personal pin")){
        alert("라켓 추가 실패: VKTC 개인 PIN 연결이 없습니다. 관리자에게 회원 목록에서 PIN을 다시 설정해주세요.");
      }else{
        alert("라켓 추가 실패: "+msg);
      }
      return;
  ...
  async function generateVktcMonthlyDues(e){
    ...
    const{data,error}=await supabase.rpc("vktc_generate_monthly_dues",{
  ...
  async function addAttendanceBatch(e){
    ...
    for(const id of newIds){
      const member=memberMap[id];
      const amount=15;
      const note=attendanceForm.note.trim()||"VKTC 게스트 참가비";
  ...
  async function settleMemberDues(memberId){
    ...
    const{error:pErr}=await supabase.from("finance_payments").insert({
      ...
      amount,
      note:`VKTC ${label} $${amount.toFixed(2)} 수납`
    });
  ...
  async function linkGlobalPlayerToVktc(player,status){
    ...
    flash(`${player.name} 선수를 VKTC ${label}으로 연결했습니다.`);
  }
  ...
  async function deactivateMember(id){
    ...
    if(!member)return;
    if(!confirm(`${member.name}을(를) VKTC에서 비활동 상태로 변경할까요?
다른 클럽의 소속/활동 상태와 글로벌 AKTR은 영향을 받지 않습니다.`))return;
  ...
  async function permanentlyDeleteInactiveMember(id){
    ...
    if(!confirm(`${member.name}을(를) VKTC 선수 목록에서 제거할까요?

공용 선수 DB 자체는 삭제하지 않습니다.
다른 클럽의 소속, 글로벌 AKTR, 다른 클럽 경기 기록은 그대로 유지됩니다.
VKTC 과거 기록이 있는 선수는 기록 보존을 위해 제거가 거부됩니다.`))return;
    ...
    await Promise.all([loadMembers(),loadClubFoundation()]);
    flash("VKTC 선수 목록에서 제거했습니다. 공용 선수 데이터는 유지됩니다.");
  }
  ...
  return <div className="app vktcApp">
    <aside className="sidebar">
      <button className="brand" onClick={()=>navigateFresh("home")} aria-label="대시보드로 이동">
        <img src="/vktc-logo.jpg" alt="VKTC - Victoria Korean Tennis Club"/>
      </button>
    ...
    <main className="main">
      {tab!=="home"&&<div className="mobileTopBar">
        <button className="mobileHomeLogo" onClick={()=>navigateFresh("home")} aria-label="홈으로 이동">
          <img src="/vktc-logo.jpg" alt="VKTC"/>
        </button>
      ...
      {tab==="home"&&<>
        <div className="mockHomeMobileBrand">
          <img src="/vktc-logo.jpg" alt="VKTC"/>
          <span><b>VKTC</b><small>VICTORIA KOREAN TENNIS CLUB</small></span>
        </div>
        ...
        <div className="mockHome">
          <section className="mockTopGrid">
            <article className="mockHero">
              <div className="mockHeroCopy">
                ...
                <div className="mockHeroMeta">
                  <div><span>♙</span><b>회원 회비</b><em>월 $30</em></div>
                  <div><span>◎</span><b>게스트 참가비</b><em>1회 $15</em></div>
                </div>

              </div>
              <div className="mockHeroImage" aria-hidden="true">
                <img src="/vktc-logo.jpg" alt=""/>
              </div>
            </article>

            <article className="mockCard vktcUpcomingHomeCard">
              <div className="mockCardHead">
                <h2>예정된 게임</h2>
                <button onClick={()=>navigate("schedule")}>전체 보기 ›</button>
              </div>

              <div className="vktcUpcomingHomeList">
                {pendingMatches.slice(0,4).map((m,i)=>{
                  const ss=sessionMap[m.session_id];
                  return <button className="vktcUpcomingHomeRow" key={m.id||i} onClick={()=>navigate("schedule")}>
                    <div className="vktcUpcomingDate">
                      <b>{String(ss?.session_date||"").slice(5).replace("-","/")||"-"}</b>
                      <small>{String(ss?.start_time||"").slice(0,5)||"--:--"}</small>
                    </div>
                    <div className="vktcUpcomingTeams">
          ...
          <section className="mockLowerGrid">
            ...
            <div className="mockRankColumn mockRankColumnSimple">
              <article className="mockCard mockCombinedHomeRank vktcHomeRankOnly">
                <section className="mockCombinedRankSection">
                  <div className="mockCardHead mockRankSectionHead">
                    <div className="mockRankTitleWrap"><h2>VKTC 랭킹 TOP5</h2></div>
          ...
          <section className="mockFooterStrip">
            ...
            <div className="mockFooterIntro">
              <b>Victoria Korean Tennis Club</b>
              <small>빅토리아 한인 테니스 커뮤니티 VKTC.<br/>실력과 관계없이 함께 즐기고 성장하는 클럽을 만들어갑니다.</small>
            </div>
      ...
      {tab==="schedule"&&<>
        <section className="surface padded vktcUpcomingIntro">
          <div className="sectionHead">
            <div>
              <h2>예정된 게임</h2>
        ...
        {pendingMatches.length>0&&<section className="surface padded">
          ...
          <div className="matchGrid vktcPendingMatchGrid">{pendingMatches.map((m,i)=>{
            const key=m.id||i,v=scoreInputs[key]||{a:"",b:""};
            const ss=sessionMap[m.session_id];
            const canScore=isAdmin||memberUnlocked;
            return <div id={`pending-match-${m.id}`} className="matchCard friendlyPendingCard vktcPendingMatchCard" key={key}>
        ...
        {pendingMatches.length===0&&<section className="surface padded">
          <div className="emptyState vktcUpcomingEmpty">
            <b>현재 예정된 게임이 없습니다.</b>
        ...
        <section className="rankingCombinedCard">
          <div className="rankingTopBar">
            <div className="rankingTopTabs">
              <button className="rankingTopTab active" type="button">VKTC 클럽 랭킹</button>
            </div>
          </div>
        ...
        {!isAdmin&&!memberUnlocked
          ...
          :<>
            ...
            {isAdmin&&<section className="surface padded">
              ...
              <div className="financeAdminGrid vktcFinanceAdminGrid">
                <form className="financeForm vktcMonthlyFeeForm" onSubmit={generateVktcMonthlyDues}>
                  <h3>회원 월회비</h3>
                  ...
                  <div className="vktcFeeRule"><b>$30</b><span>정회원 · 준회원 / 월</span></div>
                  ...
                  <div className="attendanceList">
                    {activeMembers.filter(m=>publicMemberType(m)==="guest").map(m=>{
                      ...
                      return <div className={checked?"attendanceRow selected":"attendanceRow"} key={m.id}>
                        ...
                        <span className="vktcGuestFixedFee">$15</span>
...
{tab==="profile"&&<MemberProfile member={memberMap[profileMemberId]?{...memberMap[profileMemberId],member_type:publicMemberType(memberMap[profileMemberId])}:null} matches={historyMatches} sessions={sessionMap} memberMap={memberMap} events={otrEvents} seasons={seasons} onOpenProfile={openProfile}/>}
      ...
      {tab==="settings"&&isAdmin&&<>
        <section className="surface padded">
          ...
          <div className="settingsBlock">
            <b>클럽</b><p>VKTC (Victoria Korean Tennis Club)</p>
            <b>관리자</b><p>{user?.email}</p>
            <b>클럽 관리자 권한</b><p>{clubAdminAccess?.role==="owner"?"VKTC OWNER":clubAdminAccess?.role==="admin"?"VKTC ADMIN":"권한 없음"}</p>
            ...
            <b>회원 개인 PIN</b>
            <p>회원 PIN은 클럽별로 따로 저장되고 인증됩니다. VKTC의 정회원 또는 준회원에게는 VKTC용 숫자 4자리 PIN을 설정하세요. 같은 선수가 OPEN COURT나 다른 클럽에서도 같은 번호를 원하면 같은 PIN을 사용할 수 있습니다. 다만 같은 클럽 안에서는 두 회원이 같은 PIN을 사용할 수 없습니다. VKTC PIN으로는 VKTC의 대진 생성 · 경기 결과 입력 · 회계 열람 · MY PAGE만 인증됩니다.</p>
          </div>
        </section>
        ...
        <section className="surface padded multiClubFoundationPanel">
          <div className="sectionHead">
            <div>
              <h2>공용 AKTR · VKTC 독립 운영</h2>
              <small>V11.52 VKTC Launch</small>
            </div>
          ...
          {clubFoundationError
            ...
            :<>
              <div className="multiClubStats">
                ...
                <div><small>VKTC 선수 목록</small><b>{clubFoundationSnapshot?.roster_players??members.length}</b><span>이 사이트에서만 사용하는 로스터</span></div>
                <div><small>VKTC 정회원</small><b>{openCourtRegularCount}</b><span>4개 클럽 전체에서 정회원은 1곳만 가능</span></div>
                <div><small>VKTC 준회원</small><b>{openCourtAssociateCount}</b><span>일반 화면에서는 회원으로만 표시</span></div>
              </div>
              <div className="multiClubCurrentClub">
                <b>현재 사이트</b>
                <span>{clubMap[CURRENT_CLUB_ID]?.name||"Victoria Korean Tennis Club"}</span>
                <small>Club ID · {CURRENT_CLUB_ID}</small>
              </div>
              <div className="multiClubFoundationNote">
                <b>이번 단계에서 완료된 것</b>
                <p><b>관리자 권한:</b> kkjege@hotmail.com 계정을 VKTC 관리자 초대로 등록했습니다. 해당 Supabase Auth 계정으로 로그인하면 VKTC 관리자 권한이 연결됩니다.</p>
                <p><b>VKTC 전용 영역:</b> 경기 · 회계 · 행사 · 회원소속은 VKTC Club ID로 분리됩니다. 별도 스케줄 배치 시스템은 사용하지 않고 예정된 게임 목록만 사용합니다. 시즌대회 · OPEN PICK · 명예의 전당은 사용하지 않습니다.</p>
                <p><b>회계 규칙:</b> 정회원/준회원 월 $30 · 게스트 1회 $15. 글로벌 AKTR은 다른 클럽과 그대로 공유합니다.</p>
              </div>
            </>}
        </section>
    ...
    {pinEditMember&&isAdmin&&<Modal onClose={()=>{setPinEditMember(null);setPersonalPinForm({pin:"",confirm:""});}}>
      ...
      <form className="personalPinModal" onSubmit={saveMemberPersonalPin}>
        <p>이 PIN은 VKTC 사이트에서 사용하는 클럽 전용 PIN입니다. 같은 선수가 다른 클럽에서도 같은 숫자를 원하면 같은 PIN을 사용할 수 있습니다. 단, VKTC 안에서는 다른 회원과 같은 PIN을 사용할 수 없습니다.</p>
